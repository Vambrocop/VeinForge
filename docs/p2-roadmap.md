...
## 改进路线 checklist（无需小麦/大麦数据；loop 按此挑下一项）

- [x] **单子叶纵/横脉分离 (P1.5)** — `veinforge/orient.py`，已并入 measure / DB / CSV（longitudinal_density、transverse_density、vein_axis_deg）
- [x] **更真实的合成模拟器** — `synthetic.make_realistic_phantom`（波状脉 + 光照梯度 + 染色斑 + 噪声 + 模糊）；`make_synth_pretrain.py --style realistic`
- [x] **DL 训练严谨化** — `veinforge/losses.py`(clDice 连通性损失)+ `veinforge/augment.py`(翻转/旋转/亮度增强)+ `train_dl.py --val` 固定 holdout(按最佳 val IoU 存模型)
- [x] **视网膜血管迁移** — `scripts/fetch_retinal.py`(STARE 12MB，免登录直链)→ 训 `models/retinal_unet.pt`（val IoU 0.077→**0.54**）；叶脉微调用 `train_dl.py --init models/retinal_unet.pt`（血管≈叶脉，作更通用的预训练底座）
- [x] **整叶 RGB 胁迫/健康分类（并行轨）** — `veinforge/leafstress.py`（颜色/纹理特征 + RandomForest）+ `veinforge leafstress train/predict`；吃 `<root>/<class>/图` 布局，补充叶脉轨的图像级胁迫信号（叶色/黄化/病斑）。注:PlantVillage 不含小麦,小麦病害数据在 Kaggle(需登录)。
